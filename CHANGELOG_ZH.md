...
> 版本号说明:本版**跳过 `1.0.1`** —— 该号从未推送、从未发布(其两批内容已分别并入 `1.0.0`),
> 但本地 mavenLocal 仍留有 2026-09-22 的 `1.0.1` 目录(已撤销版本的残留);复用该号属「同号覆盖」,
> 消费方可能静默解析到旧 jar(须 `--refresh-dependencies` 才能纠正),故新改动另起 `1.0.2`。

### 新功能

- **`combat/HostileTargets` 新增「额外敌对判定」注入 seam**(`ExtraHostileProbe` /
  `installExtraHostileProbe`):消费方可在启动时注入一个「某实体应当视为敌对目标」的判定体,
  使**既非 `Enemy`、也非被激怒的中立生物**的实体(典型:第三方测试假人这类靶子)能被
  「需要敌对目标」的效果正确识别。
  - 判定口径扩充为 `敌对生物 ∪ 已被激怒的中立生物 ∪ 消费方额外声明的实体`;两参重载
...
- **纯新增**:新嵌套类型 `ExtraHostileProbe` + 新静态方法 `installExtraHostileProbe` +
  既有 `isHostile(Entity)` 末尾追加一次 probe 调用;未删除/改名/改签名任何既有 public API,
  「未注入」时既有行为逐字不变 ⇒ 符合 `1.x` 兼容契约。消费方 `starengine_lib_version_range`
  的下界随本版收紧至 `[1.0.2,2.0)`。
