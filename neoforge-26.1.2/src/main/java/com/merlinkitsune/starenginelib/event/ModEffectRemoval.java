...
/**
 * 本模组主动移除效果的统一内部通道(NeoForge 26.1.2 版)。
 *
 * <p>消费方的效果移除拦截器会拦截牛奶/蜂蜜/{@code /effect clear} 等外部清除;
 * 本模组自己的移除逻辑必须经由本类移除,通过内部移除标志放行,
 * 避免"待命"提示、计数效果等被拦截后永久残留。
 *
 * <p>26.1.2 与 1.21.1 的签名一致({@code removeEffect(Holder<MobEffect>)}),
 * 与 1.20.1 侧({@code MobEffect})不同 —— 故该 shim 三个平台各一份。
 */
